...
[**Gemma 3**](https://ollama.com/library/gemma3) is very consistent in terms of performance, with errors occurring rarely. However, it is less suitable for long/complex prompts and captions. We recommend using the standard variant, `gemma3:latest`, for most use cases.

[**Qwen3-VL**](https://ollama.com/library/qwen3-vl) can be less predictable, especially in [the `2b` and `4b` variants](https://ollama.com/library/qwen3-vl/tags), where performance and error rates vary widely unless you manage to control them as shown in our [configuration example](#qwen3-vl-labels) below. The standard `qwen3-vl:latest` (`8b`) version generally behaves well, though we haven't tested it extensively. We know, however, that the previous version, Qwen2.5-VL, was [good at generating detailed captions](../../developer-guide/vision/model-comparison.md#qwen25-vl-overall-winner) in different languages and styles. The trade-off is slightly lower performance compared to Gemma 3 on an NVIDIA RTX 4060: 2–3 seconds vs 1–2 seconds for generating labels.
